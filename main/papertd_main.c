    ...
    while (1) {
        if (ESP_OK == adc_read(&adc_data)) {
			...
			if (adc_data < 1000) {
                ...
                if (MQTT_client != 0) {
                    message_id = esp_mqtt_client_publish(MQTT_client, "papertd", "Towel low in Room: 1", 0, 1, 0);
                    ESP_LOGI(TAG, "sent publish successful, message_id=%d", message_id);
                }
...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event)
{
    ...
    switch (event->event_id) {
        case MQTT_EVENT_CONNECTED:
            ...
            msg_id = esp_mqtt_client_publish(client, "papertd", "Device started", 0, 1, 0);
